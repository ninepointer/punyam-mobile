...
class _MapsScreenState extends State<MapsScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _initializeMapLocation() async {
    ...
    if (AppStorage.locationLatitude() != null && AppStorage.locationLongitude() != null) {
      initialLocation = LatLng(double.parse(AppStorage.locationLatitude()!), double.parse(AppStorage.locationLongitude()!));
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(centerTitle: true, title: Text('Confirm Location'),),
      body: _isLoading == false? Column(
        children: <Widget>[
          Expanded(
           ...
           Container(
            ...
            decoration: BoxDecoration(
              ...
              color: AppColors.cinnamonStickColor.withOpacity(1 - _animationController.value),
            ),
          ),
          // Center Marker Icon
          Icon(Icons.location_on_rounded, size: 50 ,color: AppColors.cinnamonStickColor,),
          Positioned(
            bottom: 20,
            left: 0,
            right: 0,
          ...
          Expanded(
            flex: 1, // This makes the button take 20% of the screen height
            child: Center(
              child: ElevatedButton(
                style: ElevatedButton.styleFrom(backgroundColor: AppColors.cinnamonStickColor),
                onPressed: () => {
                  BottomSheetHelper.openBottomSheet(context: context, child:AddressBottomSheet())
                } ,
